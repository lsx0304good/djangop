...
class AxfFoodtype(models.Model):
    ...
    @property
    def childtypes(self):
        ctnamelist = self.childtypenames
        ctnamelist = ctnamelist.split("#")
        ctnamelist = [value.split(":") for value in ctnamelist]
        return [{'child_name': elem[0], 'child_value': elem[1]} for elem in ctnamelist]
